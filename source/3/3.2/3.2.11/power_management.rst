Chapter 3.2.11 Power Management
===============================
...
The :guilabel:`Idle` tab manages what happens when you walk away from your keyboard. To toggle Power Management being active to see if you are not at your computer check the :guilabel:`Enable idleness watcher` checkbox. The :guilabel:`On AC` section changes how to handle idleness when your computer is plugged in. The :guilabel:`When idle` menu lets select whether you want to do Nothing, Lock Screen, Suspend, hibernate, Shutdown, or Turn Off Monitor(s) when your computer is plugged in. To change how long idle before doing what you selected on :guilabel:`When idle` use the :guilabel:`Idle time` menu. To choose an amount of idle time to turn your monitor select a time in :guilabel:`Turn off monitor(s) after` field. The :guilabel:`On Battery` Changes the settings for when your computer is on a battery. The :guilabel:`When idle` field lets you choose what to do with your computer when you leave it idle on battery. The :guilabel:`Idle time` field lets you change how long idle to wait before taking the action in :guilabel:`When idle`. To choose how long to turn of your monitors after being idle use the :guilabel:`Turn off monitor(s) after` field.
...
Version
-------
Lubuntu ships with version 2.3.0 of Power Management. 
