...
To change your settings on how transmission interacts with the network use the :guilabel:`Network` tab. To choose the the port for incoming connections to transmission use the :guilabel:`Port for incoming connections:`. To see if that port is open press the :guilabel:`Test Port` button.  To change the greatest number of peers you can have for each individual torrent change the :guilabel:`Maxiumum peers per torrent` field. To change the greateest number of peers you can have across all of you different torrents in the :guilabel:`Maximum peers overall` field.
