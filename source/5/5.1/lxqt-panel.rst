...
In the system tray there there are multiple widgets. To see more on how to use nm-tray which manages network in the tray see chapter 3.1.5. Also included is Qlipper which to see how to use view how to view chapter 2.4.5. Other Programs you install can also place a tray icon here as well. With the volume applet you can change the volume by using the mouse-wheel with the mouse over the speaker icon or left click the speaker icon and drag the volume left and right or press :kbd:`Up arrow` or :kbd:`Down arrow`. To mute the volume with the :guilabel:`speaker` button at the bottom of the volume meter or middle click on the volume applet. If you have multiple devices that can play audio the device name with its volume will listed. To see what the current volume is you can hover the mouse over the volume applet. To open pulse audio volume control you can press the :guilabel:`Mixer` button. To close the volume applet press the :kbd:`Escape` key or left click off of the volume applet.
